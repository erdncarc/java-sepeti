...
public class SignUpRestaurant1Activity extends BaseActivity {

    private EditText name, email, password;
    private CheckBox policyCheckBox;
    private TextView signupButton, loginText;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        loginText.setOnClickListener(v -> startActivity(new Intent(SignUpRestaurant1Activity.this, LoginActivity.class)));
    ...
    private void handleSignup() {
        ...
        if (fullName.isEmpty() || userEmail.isEmpty() || userPassword.isEmpty()) {
            Toast.makeText(SignUpRestaurant1Activity.this, "Please fill all fields", Toast.LENGTH_SHORT).show();
            return;
        }

        if (!isValidEmail(userEmail)) {
            Toast.makeText(SignUpRestaurant1Activity.this, "Please enter a valid email", Toast.LENGTH_SHORT).show();
            return;
        }

        if (userPassword.length() < 6) {
            Toast.makeText(SignUpRestaurant1Activity.this, "Password must be at least 6 characters long", Toast.LENGTH_SHORT).show();
            return;
        }

        if (!policyCheckBox.isChecked()) {
            Toast.makeText(SignUpRestaurant1Activity.this, "You must accept the terms and privacy policy", Toast.LENGTH_SHORT).show();
            return;
        }
    ...
    private void login(String  email, String password){
        AuthRepository.getInstance().login(email,password,new BaseCallback<LoginResponseDTO>() {
            ...
            public void onSuccess(LoginResponseDTO result) {
                Intent intent = new Intent(SignUpRestaurant1Activity.this , SignUpRestaurant2Activity.class);
            ...
            public void onError(Throwable t) {
                ...
                Intent intent = new Intent(SignUpRestaurant1Activity.this, LoginActivity.class);
                startActivity(intent);
                finish();
            }
        });
